#!/bin/bash
# tag: segment-classify-index parallel
...
# Use GNU parallel to run pure_func concurrently and append to output file
find "$IMG_DIR" -name '*.png' | sort | parallel --jobs "$MAX_PROCS" pure_func {} >> "$OUT"
